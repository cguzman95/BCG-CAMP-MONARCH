...
int compare_doubles(double *x, double *y, int len, const char *s) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if ((rel_error > tol && abs_error > 1.0E-30) || y[i] != y[i]) {
      // if(true){
      printf(
          "compare_doubles %s rel_error %le abs_error %le for tol %le at [%d]: "
          "%le vs %le\n",
...
void swapCSC_CSR(int n_row, int n_col, int *Ap, int *Aj, double *Ax, int *Bp,
                 int *Bi, double *Bx) {
  int nnz = Ap[n_row];
...
void swapCSC_CSR_BCG(ModelDataGPU *mGPU, int *Ap, int *Aj, double *Ax) {
#ifdef TEST_CSCtoCSR
...
void swapCSR_CSD(int n_row, int n_col, int *Ap, int *Aj, double *Ax, int *Bp,
                 int *Bi, double *Bx) {
  int nnz = Ap[n_row];
  ...
  for (int row = 0; row < n_row; row++) {
    for (int j = Ap[row]; j < Ap[row + 1]; j++) {
      if (Aj[j] <= row) {
        ...
        // printf("nElemTillDiag  offsetBx[iDiag] Aj[j] %d %d %d
...
void swapCSC_CSD_BCG(ModelDataGPU *mGPU, int *Ap0, int *Aj0, double *Ax0) {
#ifdef TEST_CSRtoCSD
...
void swapCSR_CUID(int n_row, int n_col, int *Ap, int *Aj, double *Ax, int *Bp,
                  int *Bi, double *Bx) {
  int nnz = Ap[n_row];
  memset(Bp, 0, (n_row + 1) * sizeof(int));
  int *Bpi = (int *)malloc((n_row) * sizeof(int));
...
void swapCSC_CSP(int n_row, int *Ai, int *Aj, double *Ax) {
  ...
  bool *isRowEmpty = (bool *)malloc(n_row * sizeof(bool));
  ...
  for (int z = 0; z < nnz; z++) {
    for (int i = 0; i < n_row; i++) { // Columns
      for (int j = Ai[i]; j < Ai[i + 1]; j++) {
        int row = Aj[j];
        int value = Ax[j];
        // printf("%s", isRowEmpty[row] ? "T" : "F");
        if (isRowEmpty[row] && row != -1) {
          // Mark row as set for next column iter
          isRowEmpty[row] = false;
          // Mark index as set for next nnz iter
          Aj[j] = -1;
          // Save index in rows matrix:
          Bj[i_B] = row;
          // Add value in nnz matrix:
          Bx[i_B] = value;
          i_B++;
          // Increase number of values for this row
          Bi[i]++;
          // Pending
    ...
    memset(isRowEmpty, 1, n_row * sizeof(bool));
  }
  if (i_B != nnz) {
    printf("ERROR: i_B!=nnz: %d!=%d %s", i_B, nnz,
           ",some values are not set in the new matrix\n");
    exit(0);
  }
  int maxDiA = 0;
  for (int i = 0; i < n_row; i++) {
...
#ifdef CUID
void swapCSR_CUID_BCG(ModelDataGPU *mGPU, int *Ai, int *Aj, double *Ax) {
  int *jA1 = (int *)malloc((n_row * n_row) * sizeof(int));
#ifdef TEST_SWAPS
  // Example configuration based in  KLU Sparse pdf
  int n_row = 3;
  int n_col = n_row;
...
void BCG() {
  ...
  fscanf(fp, "%d", &n_cells);
  fclose(fp);
  printf("confPath %s nDevices %d n_cells %d\n", confPath, nDevices, n_cells);
  ...
  int n_row = 3;
  int nnz = 6;
  int Ai2[n_row + 1] = {0, 2, 4, 6};
  int Aj2[nnz] = {0, 2, 1, 2, 0, 2};
  double Ax2[nnz] = {5., 3., 2., 1., 6., 8.};
  /*
  5 0 6
  0 2 0
  3 1 8
  */
...
#elif CUID
  printf("CUID\n");
  swapCSR_CUID_BCG(mGPU0, iA_aux, jA_aux, A_aux);
#else
  printf("CSR\n");
#endif
  ...
  int *iA = iA_aux;
  double *A =
      (double *)malloc(mGPU0->nnz * n_cells_multiplier * sizeof(double));
  double *diag =
      (double *)malloc(mGPU0->nrows * n_cells_multiplier * sizeof(double));
  double *x =
      (double *)malloc(mGPU0->nrows * n_cells_multiplier * sizeof(double));
  double *tempv =
      (double *)malloc(mGPU0->nrows * n_cells_multiplier * sizeof(double));
  ...
  for (int iDevice = 0; iDevice < nDevices; iDevice++) {
    ...
    if (remainder != 0 && iDevice == 0) {
      ...
      // printf("remainder %d n_cells_total %d nDevices %d n_cells
...
#ifdef CUID
    cudaMemcpyAsync(mGPU->djA, jA1, mGPU->lenjA * sizeof(int),
                    cudaMemcpyHostToDevice, 0);
#else
    cudaMemcpyAsync(mGPU->djA, jA, mGPU->lenjA * sizeof(int),
                    cudaMemcpyHostToDevice, 0);
#endif
    cudaMemcpyAsync(mGPU->diA, iA, mGPU0->leniA * sizeof(int),
                    cudaMemcpyHostToDevice, 0);
    cudaMemcpyAsync(mGPU->dA, A + offset_nnz, mGPU->nnz * sizeof(double),
                    cudaMemcpyHostToDevice);
    cudaMemcpyAsync(mGPU->ddiag, diag + offset_nrows,
                    mGPU->nrows * sizeof(double), cudaMemcpyHostToDevice, 0);
    cudaMemcpyAsync(mGPU->dx, x + offset_nrows, mGPU->nrows * sizeof(double),
                    cudaMemcpyHostToDevice, 0);
    cudaMemcpyAsync(mGPU->dtempv, tempv + offset_nrows,
                    mGPU->nrows * sizeof(double), cudaMemcpyHostToDevice, 0);

    solveGPU_block(mGPU);

    cudaMemcpyAsync(x + offset_nrows, mGPU->dx, mGPU->nrows * sizeof(double),
